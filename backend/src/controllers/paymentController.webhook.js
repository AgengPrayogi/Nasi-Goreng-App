...
const { handleWebhookUpdate } = require('../services/paymentService');
  ...
  try {
    ...
    if (webhookSecret) {
      const incomingSecret =
        req.headers['x-webhook-secret'] || req.headers['x-payment-secret'];
    ...
    const payment = await handleWebhookUpdate({
      orderCode,
      paymentStatus,
      paymentMethod,
      externalPaymentId,
